      ...
      <main className="max-w-6xl mx-auto p-6 md:p-10 w-full space-y-10">
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-10 md:p-16 rounded-[3.5rem] border border-slate-700 shadow-2xl text-center relative overflow-hidden">
          ...
          <div className="flex flex-col items-center gap-8">
            ...
            <button onClick={handleUpload} disabled={loading} className="bg-white text-black px-16 py-5 rounded-2xl font-black text-xl hover:bg-blue-500 hover:text-white transition-all disabled:opacity-50 flex items-center gap-3">
              {loading ? <><Loader2 className="animate-spin" /> ANALYZING...</> : "INITIALIZE SCAN"}
            </button>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-700/50">
                {user.history?.length > 0 ? user.history.map((h, i) => (
                  <tr key={i} className="hover:bg-slate-700/10 transition-colors">
                    ...
                    <td className="p-8 text-xs text-slate-500 font-bold uppercase">{h.skills?.slice(0, 3).join(" • ")}</td>
